...
summary={'completed_participants':len(sessions),'test_sessions_excluded':c.execute('SELECT COUNT(*) FROM sessions WHERE is_test=1').fetchone()[0],'incomplete_sessions_excluded':c.execute('SELECT COUNT(*) FROM sessions WHERE submitted IS NULL AND is_test=0').fetchone()[0],'preferences':[{'mode':k[0],'baseline':k[1],'criterion':k[2],**dict(v),'storymotion_win_fraction_excluding_ties_and_unjudged': v['storymotion']/(v['storymotion']+v['baseline']) if v['storymotion']+v['baseline'] else None} for k,v in sorted(counts.items())], 'interpretation':'Descriptive paired preferences, not independent per-question significance tests. Repeated observations cluster by participant and sample. The cohort was manually selected; do not generalize to the full test set.'}
